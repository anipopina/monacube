...
import { hex as hexEncoder } from '@scure/base'
...
/**
 * MonaWalletのReadOnly版に相当するクラス
 * 秘密鍵を使わない機能はこちらで実装
 */
export class MonaWalletRo {
  ...
  lastAssetBalanceUpdate: number = 0
  ...
  async updateBalance(): Promise<void> {
    const now = Date.now()
    if (now - this.lastBalanceUpdate < 10_000) return // 10秒以内の連打は無視
  ...
  mergeLocalTxos(): void {
    let utxos = [...this.utxos] // copy
    // utxosにchangeTxosをマージ
    utxos = utxos.filter((u) => {
      const changeTxo = this.changeTxos.find((change) => change.txid === u.txid && change.vout === u.vout)
      if (changeTxo && u.confirmed) changeTxo.confirmed = true // changeTxo の confirmed を更新
      return !changeTxo
    })
    utxos = utxos.concat(this.changeTxos)
    // utxosからusedTxosを除外
    utxos = utxos.filter((u) => {
      return !this.usedTxos.find((used) => used.txid === u.txid && used.vout === u.vout)
    })
    // value降順にソート
  ...
  calcBalance(): void {
    const confirmedSat = this.utxos.filter((u) => u.confirmed).reduce((sum, u) => sum + u.value, 0)
  ...
  async updateAssetBalances(): Promise<void> {
    const now = Date.now()
    if (now - this.lastAssetBalanceUpdate < 10_000) return // 10秒以内の連打は無視
    this.lastAssetBalanceUpdate = now
...
/**
 * 秘密鍵を保持してモナコインとMonapartyの処理を提供するクラス
 */
export class MonaWallet extends MonaWalletRo {
  ...
  constructor(entropy: Uint8Array, addressType: AddressType = 'P2PKH', addressPath: string = DEFAULT_ADDRESS_PATH[addressType]) {
    ...
    const child = root.derive(addressPath)
    if (!child.publicKey || !child.privateKey) throw new Error('MonaWallet: Failed to derive key')
    const privateKey = child.privateKey

    let p2xxx
    if (addressType === 'P2PKH') p2xxx = btcSigner.p2pkh(child.publicKey, MONA_NETWORK)
    else p2xxx = btcSigner.p2wpkh(child.publicKey, MONA_NETWORK)
    if (!p2xxx.address) throw new Error('MonaWallet: Failed to generate address')

    super(p2xxx.address)
    this.script = p2xxx.script
  ...
  toReadOnly(): MonaWalletRo {
    ...
    walletRo.isUnconfUtxoAvailable = this.isUnconfUtxoAvailable
    walletRo.useEsplora = this.useEsplora
    walletRo.lastBalanceUpdate = this.lastBalanceUpdate
    walletRo.assetBalances = this.assetBalances
    walletRo.lastAssetBalanceUpdate = this.lastAssetBalanceUpdate
    return walletRo
  }
  ...
  async sendMona(toAddress: string, amount: number, feeSatPerVByte: number = 200): Promise<string> {
    ...
    for (const utxo of availableUtxos) {
      if (this.addressType === 'P2PKH') {
        ...
        const prevTxHex = utxo.txHex || (await this.getTxHex(utxo.txid))
        tx.addInput({
          txid: utxo.txid,
          index: utxo.vout,
          nonWitnessUtxo: hexEncoder.decode(prevTxHex),
        })
      } else {
  ...
  updateLocalTxos(tx: btcSigner.Transaction, txId: string): void {
    ...
    for (let i = 0; i < tx.outputsLength; i++) {
      ...
      if (output.script && arraysEqual(output.script, this.script) && output.amount !== undefined) {
        this.changeTxos.push({
          txid: txId,
          vout: i,
          value: Number(output.amount),
          confirmed: false,
          txHex: hexEncoder.encode(tx.extract()),
        })
      }
    }
    // usedTxos追加 & changeTxosから削除
    for (let i = 0; i < tx.inputsLength; i++) {
      const input = tx.getInput(i)
      if (input.txid === undefined || input.index === undefined) continue
      const inputTxId = hexEncoder.encode(input.txid)
      this.changeTxos = this.changeTxos.filter((u) => !(u.txid === inputTxId && u.vout === input.index))
      this.usedTxos.push({ txid: inputTxId, vout: input.index })
    }
  }

  signMessage(message: string): string {
    const extraEntropy = Buffer.from(crypto.getRandomValues(new Uint8Array(32)))
    const signature = bitcoinMessage.sign(
      message,
      Buffer.from(this.privateKey),
      true, // compressed
      MONA_NETWORK.messagePrefix,
      { extraEntropy },
    )
  ...
  async signAndBroadcastMonapartyTxHex(txHex: string): Promise<string> {
    ...
    for (let i = 0; i < mpTx.inputsLength; i++) {
      ...
      const inputTxid = input.txid ? hexEncoder.encode(input.txid) : ''
  ...
  async sendAsset(toAddress: string, asset: string, amount: number | string, feeSatPerByte: number = 200): Promise<string> {
    ...
    const txHex = await monaparty.createSend({
      ...
      allowUnconfirmedInputs: this.useUnconfUtxo && this.isUnconfUtxoAvailable,
    })
...
function inspectMonapartyTxHex(txHex: string, sourceAddress: string): void {
  ...
  const sourceScript = btcSigner.OutScript.encode(decodedAddress)
  let totalOutputSat = 0n
  let changeBackSat = 0n
  for (let i = 0; i < tx.outputsLength; i++) {
    const output = tx.getOutput(i)
    totalOutputSat += output.amount ?? 0n
    if (output.script && arraysEqual(output.script, sourceScript)) {
      changeBackSat += output.amount ?? 0n
    }
  }
  const netOutflowSat = totalOutputSat - changeBackSat
  if (netOutflowSat > txBytes.length * MAX_OUTSAT_PER_BYTE)
    throw new Error(`Transaction sends too much MONA from your address.\nNet outflow: ${Number(netOutflowSat) / SATOSHI} MONA`)
}
